/* 
  this code is adapted from BlinkWithoutDelay 
  by Oguz Yetkin and Darcy Neal
 http://www.arduino.cc/en/Tutorial/BlinkWithoutDelay
 button allows the melody to increment through array while the button is held down, 
 and BPM is controlled by analogInput 
 */
...
int segment(int current_val, int minval, int maxval, int nsegments){
  if(nsegments <=0){  //less than or equal to
    nsegments = 1;
  }
...
//plays scale only if digitalRead = TRUE
  if(currentMillis - previousMillis > interval  && digitalRead (switchPin)) {  
    ...
    segment_to_play ++;   //increment s.t.p. by one and returns that value
